...
std::string QuadTerm::print() {
  ...
  case QuadTermKind::TEMP:
    ...
    return "t" + to_string(t_tempexp->temp->num) + ":" +
           (t_tempexp->type == Type::INT ? "INT" : "PTR");
...
#ifdef DEBUG
  cout << "In QuadProgram::print: "
       << (this->kind == QuadKind::PROGRAM ? "Program!" : "Oh?") << endl;
#endif
...
void QuadMoveExtCall::print(string &use_str, int indent,
                            bool to_print_def_use) {
#ifdef DEBUG
...
void QuadPhi::print(string &use_str, int indent, bool to_print_def_use) {
  ...
  string phi_str =
      print_indent(indent) + "PHI " + print_temp(phi_temp) + " <- (";
  bool first = true;
