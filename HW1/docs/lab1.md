...
注意事项：若有作业批改误判的问题可以私下联系助教，其他问题请直接在群里询问！！！这是因为群里有老师和其他已完成的同学可以帮忙一起解答，而且方便其他有类似问题的同学答疑解惑。
- 每次HW的目标在相应文件夹的`README.md`文件里，包括需要提供的接口以及程序预期的行为。
- 代码实现请主要参考虎书、老师课上讲的内容与PPT。文档是基于助教往年编写代码的经验抽象总结出的技巧和要点，由于每年的项目都在迭代，难免会出现错误，若有勘误请直接在群里询问。不是所有HW都有详尽的文档的，前几个HW的文档会提供保姆式的实现细节带大家熟悉项目，之后的HW不会提供具体的实现细节，需要仔细看书、认真听课、自己思考。
- 代码细节请自行理解，任何有关代码的问题请询问LLM或直接在群里询问，不要私下询问助教。
