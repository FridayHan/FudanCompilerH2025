...
- 所有问题请**直接在群里提问**（因为群里有老师和其他已完成的同学可以帮忙一起解答，而且方便其他有类似问题的同学答疑解惑），只有对作业得分有关的问题允许私下联系助教。
- 每次HW的目标在相应文件夹的 `README.md`文件里，包括程序预期的行为以及需要提供的接口。
- 每次HW请大家预留2~3个**整天**来完成（**工作量非常大**！），所有代码细节请自行理解或询问LLM或在群里问，代码实现请主要参考虎书、老师课上讲的内容（非常重要！老师上课会讲代码细节！）与PPT。
  - 不是所有HW都有详尽的文档的，前几个HW的文档会提供保姆式的实现细节带大家熟悉项目，之后的HW不会提供具体的实现细节，需要仔细看书、认真听课、自己思考。往年的课程事实上并没有提供类似的文档，前几次HW保姆式的文档均为助教当年HW的实验报告，并针对今年的改变进行微调，后续HW的编程思路都需要自己思考！
