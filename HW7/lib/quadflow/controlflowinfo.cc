...
void ControlFlowInfo::eliminateUnreachableBlocks() {
    ...
    for (auto block : unreachableBlocks) {
        // Remove block from the function's quadblocklist
#ifdef DEBUG
        cout << "Removing block: " << block << endl;
#endif
        func->quadblocklist->erase(
            std::remove_if(func->quadblocklist->begin(), func->quadblocklist->end(),
            [block](quad::QuadBlock* b) {
                return b->entry_label && b->entry_label->num == block; 
            }),
            func->quadblocklist->end()
        );
    }
