...
def compare_single_file(filename, verbose=False):
    if not filename.endswith(".4-ssa.quad"):
        filename += ".4-ssa.quad"
...
def compare_all(verbose=False):
    ...
    # 获取源文件夹中的所有.4-ssa.quad文件
    src_files = [f for f in os.listdir(SRC_DIR) if f.endswith(".4-ssa.quad")]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="比较 test 和 test/output_example 下的 .4-ssa.quad 文件差异")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--file", help="指定文件名，如 'xxx.4-ssa.quad'")
