...
public:
  ...
  ClassDecl(Pos *pos, IdExp *id, vector<VarDecl *> *vdl,
            vector<MethodDecl *> *mdl)
      : AST(pos), id(id), vdl(vdl), mdl(mdl) {}
  ClassDecl(Pos *pos, IdExp *id, IdExp *eid, vector<VarDecl *> *vdl,
            vector<MethodDecl *> *mdl)
...
public:
  ...
  Type(Pos *pos, IntExp *arity)
      : AST(pos), typeKind(TypeKind::ARRAY), arity(arity) {
...
public:
  ...
  // initialization
  VarDecl(Pos *pos, Type *type, IdExp *id) : AST(pos), type(type), id(id) {
    init = std::monostate{};
  }
...
public:
  ...
  MethodDecl(Pos *pos, Type *type, IdExp *id, vector<Formal *> *fl,
             vector<VarDecl *> *vdl, vector<Stm *> *sl)
  ...
  Formal(Pos *pos, Type *type, IdExp *id) : AST(pos), type(type), id(id) {
    if (type->typeKind == TypeKind::ARRAY) {
      if (type->arity == nullptr) {
        ...
        cerr << "Error: Array type has no arity in the formal. Not allowed!"
             << endl;
        exit(1);
      }
    }
