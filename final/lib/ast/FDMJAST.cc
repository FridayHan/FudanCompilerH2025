...
Program *Program::clone() {
  ...
  vector<ClassDecl *> *cdl_clone =
      (this->cdl) ? cloneList<ClassDecl>(this->cdl) : nullptr;
  return new Program(pos->clone(), m, cdl_clone);
}

MainMethod *MainMethod::clone() {
  vector<VarDecl *> *vdl_clone =
      (this->vdl) ? cloneList<VarDecl>(this->vdl) : nullptr;
...
ClassDecl *ClassDecl::clone() {
  IdExp *id_clone =
      (this->id) ? static_cast<IdExp *>(this->id->clone()) : nullptr;
  IdExp *eid_clone =
      (this->eid) ? static_cast<IdExp *>(this->eid->clone()) : nullptr;
  vector<VarDecl *> *vdl_clone =
      (this->vdl) ? cloneList<VarDecl>(this->vdl) : nullptr;
  vector<MethodDecl *> *mdl_clone =
      (this->mdl) ? cloneList<MethodDecl>(this->mdl) : nullptr;
  return new ClassDecl(pos->clone(), id_clone, eid_clone, vdl_clone, mdl_clone);
}

Type *Type::clone() {
  return new Type(pos->clone(), typeKind,
                  (cid) ? static_cast<IdExp *>(cid->clone()) : nullptr, arity);
}

VarDecl *VarDecl::clone() {
  IdExp *id_clone =
      (this->id != nullptr) ? static_cast<IdExp *>(id->clone()) : nullptr;
  if (holds_alternative<IntExp *>(init)) {
    return new VarDecl(pos->clone(), type->clone(), id_clone,
                       get<IntExp *>(init)->clone());
  } else if (holds_alternative<vector<IntExp *> *>(init)) {
    return new VarDecl(pos->clone(), type->clone(), id_clone,
                       cloneList<IntExp>(get<vector<IntExp *> *>(init)));
  } else {
    return new VarDecl(pos->clone(), type->clone(), id_clone);
  }
}
...
MethodDecl *MethodDecl::clone() {
  ...
  vector<Formal *> *fl_clone =
      (this->fl) ? cloneList<Formal>(this->fl) : nullptr;
  vector<VarDecl *> *vdl_clone =
      (this->vdl) ? cloneList<VarDecl>(this->vdl) : nullptr;
  vector<Stm *> *sl_clone = (this->sl) ? cloneList<Stm>(this->sl) : nullptr;
  return new MethodDecl(pos->clone(), t_clone, id_clone, fl_clone, vdl_clone,
                        sl_clone);
}
