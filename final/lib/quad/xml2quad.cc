...
static QuadStm* parseStatement(XMLElement* element) {
    ...
    if (strcmp(kind, "MOVE") == 0) {
        ...
        // 获取left_call和right_call属性值
        bool left_call = false;
        bool right_call = false;
        
        const char* leftCallStr = element->Attribute("left_call");
        if (leftCallStr && strcmp(leftCallStr, "true") == 0) {
            left_call = true;
        }
        
        const char* rightCallStr = element->Attribute("right_call");
        if (rightCallStr && strcmp(rightCallStr, "true") == 0) {
            right_call = true;
        }
        
        return new QuadMoveBinop(nullptr, dst, left, binop, right, left_call, right_call, def, use);
    } 
        ...
        // 创建参数复杂性标志向量
        vector<bool>* arg_complex = new vector<bool>(args->size(), false);
        
        // 解析对象复杂性标志
        bool obj_complex = false;
        const char* objComplexStr = element->Attribute("obj_complex");
        if (objComplexStr && strcmp(objComplexStr, "true") == 0) {
            obj_complex = true;
        }
        
        // 使用更新的构造函数
        return new QuadCall(nullptr, resultTemp, funcName, objTerm, args, arg_complex, obj_complex, def, use);
    } 
        ...
        return new QuadExtCall(nullptr, resultTemp, extFun, args, def, use);
    } 
