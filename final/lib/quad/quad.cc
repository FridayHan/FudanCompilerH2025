...
QuadMoveBinop* QuadMoveBinop::clone() const {
    ...
    return new QuadMoveBinop(node, newDst, newLeft, binop, newRight, left_call, right_call, cloneTemps(def), cloneTemps(use));
}
...
QuadCall* QuadCall::clone() const {
    TempExp* newResultTemp = result_temp ? new TempExp(result_temp->type, new Temp(result_temp->temp->num)) : nullptr;
    ...
    // 克隆arg_complex数组
    vector<bool>* newArgComplex = nullptr;
    if (arg_complex) {
        newArgComplex = new vector<bool>(*arg_complex);
    } else {
        newArgComplex = new vector<bool>();
    }
    
    return new QuadCall(node, newResultTemp, name, newObjTerm, newArgs, newArgComplex, obj_complex, cloneTemps(def), cloneTemps(use));
}
...
QuadExtCall* QuadExtCall::clone() const {
    ...
    return new QuadExtCall(node, newResultTemp, extfun, newArgs, cloneTemps(def), cloneTemps(use));
}
